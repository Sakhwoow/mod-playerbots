...
void RandomPlayerbotMgr::UpdateAIInternal(uint32 /*elapsed*/, bool /*minimal*/)
{
    ...
    {
        _isBotInitializing = sPlayerbotAIConfig.randomBotTurboInit &&
                             GameTime::GetUptime().count() < sPlayerbotAIConfig.maxRandomBots * (0.11 + 0.4);
    }
...
bool RandomPlayerbotMgr::ProcessBot(uint32 bot)
{
    ...
    if (!player)
    {
        ...
        uint32 randomBotUpdateInterval = (sPlayerbotAIConfig.randomBotTurboInit && _isBotInitializing) ? 1 : sPlayerbotAIConfig.randomBotUpdateInterval;
...
void RandomPlayerbotMgr::InitArenaTeams()
{
    ...
    if (!sPlayerbotAIConfig.randomBotArenaTeamMemberGuids.empty() && !sPlayerbotAIConfig.randomBotAccounts.empty())
    {
        ...
        CharacterDatabase.Execute(
            ...
            "WHERE c.guid IN ({}) "
            "AND gm.guildid IN ("
            "  SELECT DISTINCT sub.gid FROM ("
            "    SELECT gm2.guildid AS gid FROM guild_member gm2 "
            "    JOIN characters c2 ON c2.guid = gm2.guid "
            "    JOIN acore_auth.account a2 ON a2.id = c2.account AND a2.username NOT LIKE 'rndbot%%'"
            "  ) sub"
            ")",
            arenaGuidList);
...
bool RandomPlayerbotMgr::IsRandomBot(ObjectGuid::LowType bot)
{
    ...
    if (!IsRndBotAccount(sCharacterCache->GetCharacterAccountIdByGuid(guid)))
        return false;
