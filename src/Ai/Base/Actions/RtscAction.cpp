...
bool RTSCAction::Execute(Event event)
{
    ...
    if (command != "reset" && !master->HasSpell(RTSC_MOVE_SPELL))
    {
        ...
        botAI->TellMasterNoFacing("RTS управление включено.");
        botAI->TellMasterNoFacing("Заклинание перемещения обучено.");
    }
    else if (command == "reset")
    {
        if (master->HasSpell(RTSC_MOVE_SPELL))
        {
            ...
            botAI->TellMasterNoFacing("RTS управление отключено.");
        }
