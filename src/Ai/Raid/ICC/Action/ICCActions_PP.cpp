...
bool IccPutricideAvoidMalleableGooAction::Execute(Event /*event*/)
{
    ...
    if (bot->FindNearestCreature(NPC_VOLATILE_OOZE, 200.0f) || bot->FindNearestCreature(NPC_GAS_CLOUD, 200.0f))
        return false;
...
bool IccPutricideAvoidMalleableGooAction::HandleTankPositioning(Unit* boss)
{
    ...
    if (boss && boss->IsAlive() &&
        !bot->FindNearestCreature(NPC_VOLATILE_OOZE, 200.0f) && !bot->FindNearestCreature(NPC_GAS_CLOUD, 200.0f))
    {
        if (Group* group = bot->GetGroup())
        {
            constexpr uint8 skullIconId = 7;
